...
- [ ] Supprimer le rôle `SPECTATOR` de `GamePlayerRole` (garder seulement `HOST` et `PLAYER`)
...
- [ ] `game-player.entity.ts` - Supprimer le champ `seatOrder` (inutile avec 2 joueurs)
...
- [ ] `enums.ts` - Supprimer de `SPECTATOR` du `GamePlayerRole` enum
